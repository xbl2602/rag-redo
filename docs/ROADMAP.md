...
3. ~~`env_bootstrap` 的真正执行（Phase 2 遗留）~~——**已完成（2026-09-23）**：`core/subprocess_service.py::resolve_plugin_python()` 现在真的会在约定路径找不到独立 venv 时跑一遍插件声明的 `env_bootstrap` 脚本；`official-visual-wemm` 补上了真实的 `env_bootstrap.py`+`requirements.txt`（torch/transformers/qwen_vl_utils/torchvision）并在 `plugin.toml` 里声明。测试套件用新增的 `RAG_REDO_SKIP_ENV_BOOTSTRAP` 环境变量跳过真实建环境这一步（同 `RAG_REDO_FAKE_WEMM` 一个纪律：测试不碰真实网络/不装几GB重依赖）。**尚未做的是一次真实的干净安装验证**——这台机器上没有实跑过"删掉 `plugins/official-visual-wemm/.venv` 后重新 enable 一次插件、看着它真的从零 pip install 成功"这个端到端过程（本轮机制层面用一个不装真实重依赖的临时脚本验证过"核心真的会跑脚本、真的按约定路径重新探测"这条编排逻辑本身是对的，见 `tests/test_subprocess_service.py::TestResolvePluginPythonEnvBootstrap`，但没有跑一遍真实的 torch 安装，那需要几分钟+几GB下载，本轮没有专门跑这一步）。
4. **给 `official-ocr-mineru-local` 接真实模型（Phase 2）**——当前子进程里 `_real_ocr()` 是懒导入+清楚报错的占位，`RAG_REDO_FAKE_OCR=1` 才是测试走的路径，没有装、也没有下载任何真实OCR依赖/权重。GPU仲裁+软驱逐协议+env_bootstrap机制都已经就绪（见上两条），这个插件现在唯一缺的就是：①选定具体依赖（README 里占位写的是 `mineru`，需要确认实际包名/安装方式）；②在 `_real_ocr()` 里接真实调用；③补上这个插件自己的 `requirements.txt`+`env_bootstrap.py`（结构可以直接照抄 `official-visual-wemm` 那两个文件）。
5. **Phase 3 剩余两项**——库 AI 摘要（`official-library-summary`，需要先设计 `llm_provider` 扩展点+有序回退链，AGENTS.md"插件规则"一节已经写了设计意图）、Agent 写权限门禁通用化（`core/write_gate.py` 机制已实现，但还没有任何插件真的调用它触发）。
6. **Phase 4：Windows 安装包**——PyInstaller 打包这一半已经在真实 Windows 11 机器上做完并验证过（见下方 Phase 4 状态段落），免安装 onedir 产物能跑；**还没做的是 Inno Setup 安装包包装本身**（开始菜单快捷方式/卸载入口）+ 在一台"没装过任何开发工具"的干净 Windows 机器上验证打包产物能跑（目前只在打包机器本机验证过）+ 安装/卸载前后系统关键位置无残留的实测。
