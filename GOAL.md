...
- [ ] C2 MCP 工具集对齐16个（当前覆盖12/16：list_libraries/get_library_sample/propose_library_summary/apply_library_summary/search_knowledge/reindex_knowledge/index_status/navigate_knowledge/wemm_status/read_document/find_duplicates/note_relations；`export_library`/`import_library` 是 rag-redo 独有额外工具，不计入16个对齐范围。仍缺4个：`get_selection`/`propose_selection_changes`/`apply_selection_changes`（B类选择写权限门禁）、`index_failures`（简化版）——以 `test_tools_are_discoverable_with_schema` 的实际断言集合为准）
